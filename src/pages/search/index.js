...
function Search() {
  ...
  useEffect(() => {
    const get = async () => {
      const response = await search(categoryId, title);
      console.log("====================================");
  ...
  return (
    <LayoutSix query={{ categoryId, title }}>
      ...
      <div className="shop-content space-pt--50 space-pb--50">
        <div className="custom-container5">
          <Row>
            <Col lg={10} className="card">
              <div class="pageTitle">
                <h1>{title} </h1>
                <span>({searchResult?.length} Items)</span>
              </div>
              {/* shop page header */}
              <ShopHeader
                getLayout={getLayout}
              ...
              />
              {/* shop products */}
              <ShopProducts layout={layout} products={searchResult} />
            ...
            <Col lg={2} className="order-lg-first mt-4 pt-2 mt-lg-0 pt-lg-0">
              {/* sidebar */}
              <Sidebar products={searchResult} getSortParams={getSortParams} />
            </Col>
          </Row>
        </div>
      </div>
